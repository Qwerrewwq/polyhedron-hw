...
int main() {
    ...
    while (fgets(line, sizeof(line), file)) {
        ...
        if (line_num == 1) {
            ...
            printf("Processing facet with %d vertices\n", size);
    ...
    if (facet_count > 0) {
        printf("First facet has %d vertices, vertexes ptr=%p\n", 
               facets[0].vertex_count, (void*)facets[0].vertexes);
    }
    
    /* Check first edge */
    if (edge_count > 0) {
        printf("First edge: gaps=%d, gaps_ptr=%p\n", 
               edges[0].gaps_count, (void*)edges[0].gaps);
    }
    
    printf("\nDone without freeing!\n");
